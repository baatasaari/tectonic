...
from agent_marketplace.db.session import make_engine


def test_make_engine_applies_configured_pool_settings():
    settings = AgentMarketplaceSettings(database_url="postgresql+asyncpg://u:p@localhost:5432/db")
...
def test_pool_settings_are_overridable_via_settings():
    settings = AgentMarketplaceSettings(
