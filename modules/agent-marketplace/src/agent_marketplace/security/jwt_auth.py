...
- `mint_service_token` / `ServiceBearerAuth`: the outbound side. This
  module's one platform-peer outbound dependency (Agent Cards) is
  called with a token scoped to it via the `aud` claim.
- `ServiceAuthMiddleware`: the inbound side. Verifies every request's
  ...
  module makes). This module has no externally-facing surface — every
  route it exposes is platform-internal.
"""
...
from agent_marketplace.telemetry.logging import get_logger
