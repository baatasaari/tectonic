...
(Evaluation Framework, Regulatory Compliance).
...
calls and get retry + circuit-breaking for free. Both this module's
outbound dependencies are real, fixed-base_url platform peers, so
neither client needs MCP/A2A's own per-target-URL breaker variant.
"""
