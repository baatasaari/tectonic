...
- `mint_service_token` / `ServiceBearerAuth`: the outbound side. This
  module's two platform-peer outbound dependencies (Evaluation Framework,
  Regulatory Compliance) are each called with a token scoped to that
  peer via the `aud` claim.
- `ServiceAuthMiddleware`: the inbound side. Verifies every request's
  ...
  module makes). Every route this module exposes is platform-internal —
  unlike MCP or A2A, this module has no externally-facing surface, so
  there's no third exclusion category to carve out here.
"""
