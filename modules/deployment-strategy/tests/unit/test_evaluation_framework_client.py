...
async def test_list_scores_returns_the_items_array():
    route = respx.get("http://evalfw.local/v1/evaluation-framework/scores").mock(
        return_value=httpx.Response(200, json={
            "items": [{"id": "s1", "metric_name": "groundedness", "score": 0.9, "threshold": 0.8, "passed": True, "created_at": "2026-01-01T00:00:00Z"}],
            "total": 1, "limit": 200, "offset": 0,
        })
    )
    client = HTTPEvaluationFrameworkClient("http://evalfw.local")

    scores = await client.list_scores(tenant_id="acme", agent_ref="deployment:svc:v1")

    assert len(scores) == 1
    sent = route.calls.last.request
    assert "agent_ref=deployment%3Asvc%3Av1" in str(sent.url)


@respx.mock
async def test_list_scores_returns_empty_list_when_the_deployment_has_no_history():
