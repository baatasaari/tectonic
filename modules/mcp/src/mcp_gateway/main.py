...
from mcp_gateway.api.routes_mcp import router as mcp_router
from mcp_gateway.app_context import AppContext
from mcp_gateway.clients.mcp_backend_client import MCPBackendHTTPClient
from mcp_gateway.config import MCPGatewaySettings, load_settings
...
def build_app_context(settings: MCPGatewaySettings) -> AppContext:
    engine = make_engine(settings)
    return AppContext(
        settings=settings,
        engine=engine,
        session_factory=make_session_factory(engine),
        backend=MCPBackendHTTPClient(),
    )


@asynccontextmanager
async def lifespan(app: FastAPI):
    settings: MCPGatewaySettings = app.state.settings
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="MCP",
        version="0.1.0",
        description="Tectonic Agentic AI Platform — Module 21: standardised agent-to-tool/data "
        "interface over JSON-RPC, with an internal server marketplace and per-tenant/per-tool "
        "access policy.",
        lifespan=lifespan,
    )
