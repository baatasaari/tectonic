...
- `mint_service_token` / `ServiceBearerAuth`: the outbound side. Every HTTP
  ...
  different one, limiting the blast radius of a leaked/intercepted token
  to the one peer it was meant for. This module has no platform-peer
  outbound dependency (its one outbound call target is an arbitrary
  registered MCP server, deliberately excluded — see
  clients/mcp_backend_client.py), so `ServiceBearerAuth` is included here
  for interface consistency with the rest of the platform but currently
  unused by this module's own wiring.
...
from mcp_gateway.telemetry.logging import get_logger
