"""Resiliency for this module's outbound HTTP calls to registered MCP
backend servers.
...
circuit-breaking on tool calls; this applies the same pattern here).
...
`clients/` should inherit from: `self._get(path, ...)` /
`self._post(path, ...)` replace direct `self._client.get/post(...)`
calls and get retry + circuit-breaking for free.
"""
