...
to a fifth module -- real OpenAPI-schema-driven fuzzing against this
...
skips rather than fails. Also needs a real Redis (this module's
semantic cache and quality-score store) -- see `redis_url` below.
"""
...
def contract_app(migrated_url):
    ...
    context manager, since schemathesis's ASGI transport DOES drive the
    ASGI lifespan protocol (the real `lifespan()` would otherwise
    overwrite app.state.ctx with its own freshly-built context on every
    single case.call(), silently undoing everything this fixture sets
    up -- the same fix Workflow Engine's/Multi-tenancy's/Vector DB's
    own contract conftest.py apply for the identical reason). `redis`
    needs to be genuinely reachable (`localhost:6379` by default, this
    module's own semantic cache and quality-score store) -- point
    `LLM_GATEWAY_REDIS_URL` at a real one if it isn't already running.
    `multi_tenancy` is swapped for this module's own `StubMultiTenancyQuotaClient`
    (ticket #78's quota pre-flight check runs on every completion, and
    a real Multi-tenancy being unreachable is not the input-validation
    contract this tier proves)."""
    from contextlib import asynccontextmanager

    from sqlalchemy.ext.asyncio import async_sessionmaker, create_async_engine
    from sqlalchemy.pool import NullPool

    from llm_gateway.core.fakes import StubMultiTenancyQuotaClient
    from llm_gateway.main import build_app_context, create_app
...
def auth_headers(contract_app):
    ...
    from llm_gateway.security.jwt_auth import mint_service_token
