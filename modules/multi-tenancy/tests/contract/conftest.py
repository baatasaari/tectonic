...
to a third module -- real OpenAPI-schema-driven fuzzing against this
...
def contract_app(migrated_url):
    ...
    context manager. This module's own real `lifespan` also starts a
    real Kafka producer and a background outbox-relay worker task
    (ticket #79's event-backbone rollout) -- and schemathesis's ASGI
    transport DOES drive the ASGI lifespan protocol, so leaving the
    real `lifespan` wired in would make every contract-test run
    actually try to connect to Kafka (and leave a worker task
    running). Neither is needed to exercise this module's own
    request-handling contract, so the app's lifespan is swapped for a
    no-op before use -- the same fix Workflow Engine's own contract
    conftest.py applies for the identical reason."""
    from contextlib import asynccontextmanager

    from sqlalchemy.ext.asyncio import async_sessionmaker, create_async_engine
    from sqlalchemy.pool import NullPool

    from multi_tenancy.main import build_app_context, create_app
...
def auth_headers(contract_app):
    ...
    from multi_tenancy.security.jwt_auth import mint_service_token
