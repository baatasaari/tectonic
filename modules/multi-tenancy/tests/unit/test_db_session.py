...
from multi_tenancy.db.session import make_engine


def test_make_engine_applies_configured_pool_settings():
    settings = MultiTenancySettings(database_url="postgresql+asyncpg://u:p@localhost:5432/db")
...
def test_default_probe_targets_include_agent_cards():
    settings = MultiTenancySettings()

    names = [t.name for t in settings.probe_targets]

    assert "agent-cards" in names
