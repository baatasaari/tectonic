...
class OrganisationService:
    ...
    async def _transition(self, organisation_id: str, to_status: HierarchyStatus) -> OrganisationRecord:
        ...
        org.version += 1
        org.updated_at = now()
        updated = await self._repository.update_organisation(org)
        await self._auditability.emit({
    ...
    async def suspend(self, organisation_id: str, *, reason: str) -> OrganisationRecord:
        ...
        return await self._transition(organisation_id, HierarchyStatus.SUSPENDED)

    async def reactivate(self, organisation_id: str) -> OrganisationRecord:
        return await self._transition(organisation_id, HierarchyStatus.ACTIVE)

    async def delete(self, organisation_id: str) -> OrganisationRecord:
        ...
        return await self._transition(organisation_id, HierarchyStatus.DELETED)
