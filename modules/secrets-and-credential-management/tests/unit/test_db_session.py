...
from secrets_and_credential_management.db.session import make_engine


def test_make_engine_applies_configured_pool_settings():
    settings = SecretsAndCredentialManagementSettings(database_url="postgresql+asyncpg://u:p@localhost:5432/db")
...
def test_secrets_master_key_is_distinct_from_jwt_shared_secret_by_default():
    settings = SecretsAndCredentialManagementSettings()

    assert settings.secrets_master_key != settings.jwt_shared_secret
