...
`security/jwt_auth.py` docstring for the full rationale). Not the same
key or purpose as this module's own `security/envelope_encryption.py`
-- see that module's docstring for why the two are deliberately kept
separate.

- `mint_service_token` / `ServiceBearerAuth`: the outbound side. This
  module's two platform-peer outbound dependencies (Identity and
  Access, Auditability) are each called with a token scoped to it via
  the `aud` claim.
