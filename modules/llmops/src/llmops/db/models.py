"""SQLAlchemy 2.0 declarative models for the LLMOps module data model
(LLD §3): ModelVersion, Deployment.
"""
...
from llmops.db.base import Base
...
class ModelVersion(Base):
    ...
    __table_args__ = (Index("ix_model_versions_tenant_model", "tenant_id", "model_name"),)
...
class Deployment(Base):
    ...
    __table_args__ = (
        Index("ix_deployments_tenant_model_target", "tenant_id", "model_name", "target"),
        Index("ix_deployments_stage", "stage"),
    )

    id: Mapped[str] = mapped_column(UUIDType, primary_key=True, default=_new_id)
    tenant_id: Mapped[str] = mapped_column(String(255))
    model_version_id: Mapped[str] = mapped_column(UUIDType)
    model_name: Mapped[str] = mapped_column(String(255))
