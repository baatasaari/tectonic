...
- `mint_service_token` / `ServiceBearerAuth`: the outbound side. This
  module's one platform-peer outbound dependency (Evaluation Framework)
  is called with a token scoped to it via the `aud` claim.
...
from llmops.telemetry.logging import get_logger
