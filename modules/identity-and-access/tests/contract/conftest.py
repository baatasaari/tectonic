"""Shared fixture for the contract-test tier (P0 Phase 1A closure item:
rollout of Billing and Metering's own reference implementation, ticket
#73/#80, to Identity and Access -- real OpenAPI-schema-driven fuzzing
against this module's own real, running app, not just an
installed-but-unused `schemathesis` dev dependency). Reuses the exact
same real-Postgres provisioning logic `tests/integration/conftest.py`
already established for this module -- see that file's own docstring
for the two ways a real Postgres gets obtained, and why neither
available means this tier skips rather than fails.
"""
...
def contract_app(migrated_url):
    ...
    background-worker side effects (no Kafka producer/outbox relay,
    unlike Multi-tenancy/Workflow Engine) -- the OIDC verifier's
    `httpx.AsyncClient` and the DB engine are both lazy, nothing opens a
    real connection at construction time -- but the lifespan is still
    swapped for a no-op here for the same reason those modules' contract
    conftest.py files do: consistency with the one established pattern,
    and schemathesis's ASGI transport does drive the ASGI lifespan
    protocol regardless of whether a given module's own lifespan has
    anything worth avoiding."""
    from contextlib import asynccontextmanager

    from sqlalchemy.ext.asyncio import async_sessionmaker, create_async_engine
    from sqlalchemy.pool import NullPool

    from identity_and_access.main import build_app_context, create_app
...
def auth_headers(contract_app):
    ...
    own audience -- every route this module serves under
    `/v1/identity-access/*`, `/openapi.json` itself included
    (`ServiceAuthMiddleware` excludes only `/healthz`/`/metrics`, a
    deliberately zero-trust posture: this module's API shape isn't
    handed out unauthenticated either), sits behind it. SCIM's own
    routes (`/scim/v2/*`) are deliberately excluded from this tier --
    see `api_schema`'s own docstring below."""
    from identity_and_access.security.jwt_auth import mint_service_token
...
def api_schema(contract_app, auth_headers):
    ...
    middleware stack (`ServiceAuthMiddleware`) and real route handlers,
    not a mocked transport. `headers` here authenticates the schema
    *fetch* itself; `test_openapi_contract.py` passes the same headers
    again on every generated operation call.

    SCIM's own routes (`api/routes_scim.py`, `/scim/v2/{tenant_id}/...`)
    are excluded here -- they authenticate via a completely different
    mechanism (`security/scim_auth.py`'s per-tenant bearer token, not
    `ServiceAuthMiddleware`'s shared secret this fixture's `auth_headers`
    carries), so fuzzing them with this module's own service-to-service
    token would only prove they correctly reject it, never exercise
    their real input-validation contract. A SCIM-specific contract tier
    (its own token-minting fixture) is a reasonable future addition, out
    ...
    return schema.exclude(path=["/healthz", "/metrics"]).exclude(path_regex=r"^/scim/")
