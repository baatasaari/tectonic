...
from identity_and_access.db.session import make_engine


def test_make_engine_applies_configured_pool_settings():
    settings = IdentityAndAccessSettings(database_url="postgresql+asyncpg://u:p@localhost:5432/db")
...
def test_token_signing_secret_is_distinct_from_jwt_shared_secret_by_default():
    settings = IdentityAndAccessSettings()

    assert settings.token_signing_secret != settings.jwt_shared_secret
