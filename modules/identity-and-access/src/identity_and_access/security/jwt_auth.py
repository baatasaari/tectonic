...
story, and it is NOT the same secret or token shape as this module's
own `security/token_signer.py` -- see that module's docstring for why
the two are deliberately kept separate.

- `mint_service_token` / `ServiceBearerAuth`: the outbound side. This
  module's one platform-peer outbound dependency (Auditability) is
