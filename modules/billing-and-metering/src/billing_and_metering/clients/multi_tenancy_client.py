...
`PricingPlanService.create` gives that tenant a plan of its own, so
`gate(tenant_id, module=...)` calls elsewhere in the platform reflect
what the tenant is actually paying for.

This sync is deliberately best-effort: a plan is a real, committed
billing record the moment it's created, and Multi-tenancy being briefly
unreachable must never block or fail that. `sync_entitlements` swallows
its own errors (logs a warning, returns) rather than raising -- callers
never need a try/except around it. This mirrors the entitlement gate's
own documented fail-open posture: a commercial/entitlement path must
never become an availability dependency for a module that owns none of
