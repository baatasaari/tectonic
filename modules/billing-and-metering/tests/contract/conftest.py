"""Shared fixture for the contract-test tier (Phase 1 kernel: CI
supply-chain gates -- real OpenAPI-schema-driven fuzzing against this
...
def contract_app(migrated_url):
    ...
    from billing_and_metering.main import build_app_context, create_app

    app = create_app()
    app.state.ctx = build_app_context(app.state.settings)
    return app
...
def auth_headers(contract_app):
    ...
    from billing_and_metering.security.jwt_auth import mint_service_token
