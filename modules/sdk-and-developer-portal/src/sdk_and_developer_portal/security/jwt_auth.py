...
- `mint_service_token` / `ServiceBearerAuth`: the outbound side. This
  module's platform-peer outbound dependencies (Identity and Access,
  Multi-tenancy, Auditability, and every catalogued module's own
  `/openapi.json`) are each called with a token scoped to it via the
  `aud` claim.
