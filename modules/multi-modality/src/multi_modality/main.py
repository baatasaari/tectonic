...
from multi_modality.api.routes_multi_modality import router as multi_modality_router
...
from multi_modality.config import MultiModalitySettings, load_settings
...
def build_app_context(settings: MultiModalitySettings) -> AppContext:
...
async def lifespan(app: FastAPI):
    settings: MultiModalitySettings = app.state.settings
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        ...
        version="0.1.0",
        description="Tectonic Agentic AI Platform — Module 28: text, voice, image and document "
        "extraction unified behind one interface, with a real cross-modal groundedness gate via Guardrails.",
        lifespan=lifespan,
    )
    ...
    app.include_router(multi_modality_router)
