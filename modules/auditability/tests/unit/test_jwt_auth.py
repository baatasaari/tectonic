...
underlying functions — actually rejects/accepts requests correctly. Also
covers this module's own extension: `caller_service_name`, which every
`POST /events` route depends on to resolve `source_module` from the
...
def _protected_app(*, audience: str, shared_secret: str) -> FastAPI:
    ...
    async def whoami(request: Request):
        return {"caller": caller_service_name(request)}
...
def test_middleware_rejects_a_token_scoped_to_a_different_service():
    app = _protected_app(audience="this-service", shared_secret=SECRET)
    # Minted for a *different* peer -- must not work here even though the secret matches,
    # limiting a leaked/replayed token's blast radius to the one peer it was meant for.
...
def test_caller_service_name_resolves_the_verified_issuer():
    app = _protected_app(audience="auditability", shared_secret=SECRET)
    token = mint_service_token(issuer="workflow-engine", audience="auditability", shared_secret=SECRET)
    with TestClient(app) as client:
        resp = client.get("/whoami", headers={"Authorization": f"Bearer {token}"})

    assert resp.json() == {"caller": "workflow-engine"}
