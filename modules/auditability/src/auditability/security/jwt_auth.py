...
**Module-specific addition vs. the reference implementation replicated
elsewhere on this platform**: on success, the verified claims are stashed
on `request.state.service_claims` — this module is the one place on the
platform where a route handler needs the *caller's own identity* (the
`iss` claim) for something other than authorization: `source_module` on
every ingested audit event is read from here, never from the request body,
so a caller cannot misattribute an event to a different source (see the
...
class ServiceAuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        try:
            claims = verify_service_token(token, audience=self._audience, shared_secret=self._shared_secret)
...
def caller_service_name(request: Request) -> str:
    """The verified caller's own identity (JWT `iss` claim), stashed by
    `ServiceAuthMiddleware` above. Used wherever this module needs to know
    *who* is calling for something other than authorization -- see the
    module docstring."""
    claims = getattr(request.state, "service_claims", None)
    if not claims:
        # Only reachable if this is called on an excluded path (/healthz, /metrics),
        # which never carry a token -- or in a test hitting a route directly without
