...
Shared-signing-key design: every module holds the same HS256 secret
(`TECTONIC_JWT_SHARED_SECRET` — one Kubernetes Secret, referenced by
every module's Helm chart under that same env var name) and both mints
and verifies tokens with it. This is service-to-service auth for
inter-module calls; it is not the platform's external-facing user-auth
story.

- `mint_service_token` / `ServiceBearerAuth`: the outbound side. This
  module's one platform-peer outbound dependency (Workflow Engine, see
  `clients/workflow_engine_client.py`) is called with a token scoped to
  it via the `aud` claim.
- `ServiceAuthMiddleware`: the inbound side. Verifies every request's
  ...
  module makes), `/.well-known/agent.json` (Agent Card discovery has to
  work before a caller has any credential — that's the point of a
  well-known endpoint), and `/v1/a2a/rpc` (the actual A2A wire surface
  external, third-party agents call into — they were never issued this
  platform's shared secret; that endpoint is instead gated by the
  module's own Access Policy Engine, per-caller, deny-by-default — see
  the module README's "Design notes vs. the LLD"). Every *other* route
  under `/v1/a2a/*` (`/delegate`, `/tasks/*`) is platform-internal and
  stays behind this middleware.
"""
...
from a2a_gateway.telemetry.logging import get_logger
...
_EXCLUDED_PATHS = frozenset({"/healthz", "/metrics", "/.well-known/agent.json", "/v1/a2a/rpc"})
