...
underlying functions — actually rejects/accepts requests correctly, and
that the module's public/well-known-style routes stay excluded.
"""
...
def test_middleware_excludes_healthz_metrics_and_public_a2a_routes_from_auth():
    ...
    with TestClient(app) as client:
        ...
        assert client.post("/v1/a2a/rpc", json={"jsonrpc": "2.0", "method": "message/send"}).status_code == 200


def test_middleware_rejects_a_platform_internal_route_without_a_token():
    app = _protected_app(audience="this-service", shared_secret=SECRET)
    with TestClient(app) as client:
        resp = client.get("/v1/a2a/tasks/1")
...
def test_middleware_accepts_a_valid_token_scoped_to_this_service():
    ...
    with TestClient(app) as client:
        resp = client.get("/v1/a2a/tasks/1", headers={"Authorization": f"Bearer {token}"})
...
async def test_service_bearer_auth_attaches_a_valid_bearer_header_to_outbound_requests():
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://peer", auth=auth) as client:
        resp = await client.get("/v1/a2a/tasks/1")
