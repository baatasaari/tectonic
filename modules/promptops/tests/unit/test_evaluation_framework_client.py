...
async def test_list_scores_returns_the_items_array():
    ...
    scores = await client.list_scores(tenant_id="acme", agent_ref="prompt:claims-summariser:1")

    assert len(scores) == 1
    sent = route.calls.last.request
    assert "agent_ref=prompt%3Aclaims-summariser%3A1" in str(sent.url)


@respx.mock
async def test_list_scores_returns_empty_list_when_no_history():
