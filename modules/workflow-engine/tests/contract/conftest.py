...
to a second module -- real OpenAPI-schema-driven fuzzing against this
...
def contract_app(migrated_url):
    ...
    context manager. Unlike billing-and-metering (the original
    reference implementation, which carries no lifespan side effects
    worth worrying about), this module's own real `lifespan` also
    starts a real Kafka producer and a background outbox-relay worker
    task -- and schemathesis's ASGI transport DOES drive the ASGI
    lifespan protocol, so leaving the real `lifespan` wired in would
    make every contract-test run actually try to connect to Kafka (and
    leave a worker task running). Neither is needed to exercise this
    module's own request-handling contract (the outbox insert itself
    is a plain DB write inside `update_instance_and_enqueue_event`;
    relay to Kafka is a separate, deliberately-unstarted concern here),
    so the app's lifespan is swapped for a no-op before use."""
    from contextlib import asynccontextmanager

    from workflow_engine.main import build_app_context, create_app
    ...
    ctx, _event_publisher = build_app_context(app.state.settings)
    ...
    # across calls leaks a connection (and logs a real "Event loop is closed" error
    # on each failed return-to-pool) every single call, exhausting Postgres's
...
def auth_headers(contract_app):
    ...
    from workflow_engine.security.jwt_auth import mint_service_token
