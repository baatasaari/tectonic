...
from workflow_engine.core.fakes import InMemoryEventPublisher, InMemoryWorkflowRepository
...
async def _seed_pending_event(repository) -> WorkflowInstanceRecord:
    instance = await repository.create_instance(
        WorkflowInstanceRecord(
            id=new_id(), definition_id="def-1", definition_version=1, tenant_id="acme",
            trace_id="trace-1", context={},
        )
    )
    envelope = events.workflow_started("acme", "trace-1", instance.id, "def-1")
    await repository.update_instance_and_enqueue_event(
...
async def test_poll_once_claims_and_publishes_the_oldest_pending_event():
    ...
    assert publisher.published == [(events.TOPIC_WORKFLOW_INSTANCE, envelope)]


async def test_poll_once_returns_false_when_the_queue_is_empty():
    worker = _worker(InMemoryWorkflowRepository())
...
def test_event_outbox_record_defaults_to_pending():
    record = EventOutboxRecord(id="e1", topic="workflow.instance", tenant_id="acme", envelope={})
