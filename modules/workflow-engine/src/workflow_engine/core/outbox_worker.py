...
`WorkflowRepository.update_instance_and_enqueue_event` durably queued,
to Kafka via the existing `EventPublisher` port.

Reuses this platform's own established durable-background-job shape --
the same one Regulatory Compliance's `EvidencePackWorker` already built
for evidence-pack generation, applied here to `event_outbox` instead of
`evidence_packs`:

- `WorkflowRepository.claim_next_outbox_event` atomically claims the
  oldest pending event via `SELECT ... FOR UPDATE SKIP LOCKED` in the
  real SQL implementation -- the row lock is what lets multiple worker
  processes/pods poll the same table concurrently without two of them
...
- `fail_exhausted_outbox_events` is the poison-pill guard: an event
  that has failed `max_attempts` times in a row stops being retried and
  is marked `failed` for good (a malformed envelope, or a Kafka topic
  ACL problem, must not block the whole queue forever).
...
RepositoryFactory = Callable[[], AbstractAsyncContextManager[WorkflowRepository]]
...
class OutboxRelayWorker:
    def __init__(
        ...
        *,
        ...
        # A factory over the WorkflowRepository port, not a raw DB session factory --
