"""Resiliency for this module's outbound HTTP calls to peer modules.
...
standard "Release It!" pattern — Tool Orchestration's own LLD calls for
circuit-breaking on tool calls; this applies the same pattern to every
module's own outbound calls to a peer, not just Tool Orchestration's).
...
`ResilientHTTPClient` is the base class every client in this module's
`http_clients.py` should inherit from: `self._get(path, ...)` /
`self._post(path, ...)` replace direct `self._client.get/post(...)`
calls and get retry + circuit-breaking for free. Streaming calls
(`.stream()`) are deliberately NOT wrapped here — retrying a
partially-consumed stream is unsafe; a streaming client should handle
its own reconnect logic if it needs one.
"""
...
class ResilientHTTPClient:
    """Base class for this module's outbound HTTP clients. Every call made
    through `_get`/`_post`/`_put`/`_delete` gets exponential-backoff retry
    on transient failures and a per-client circuit breaker that opens
    after repeated failures — so a struggling peer gets a break instead
    of a retry storm, and callers fail fast instead of piling up on a
    peer that's already down."""
    ...
    async def _get_optional(self, path: str, **kwargs: Any) -> httpx.Response | None:
        ...
        endpoints where "not found" is an expected outcome, not an error. Still
        retried and breaker-tracked like any other call; only the final 404 is
        treated as non-exceptional."""
        try:
