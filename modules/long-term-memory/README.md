...
- **NUL bytes in a raw string query parameter reaching the database
  ...
  sweep was otherwise done). `GET /reflections`'s `agent_ref` never ran
  through a NUL-byte validator — a plain, un-wrapped `str` function
  ...
  this module before this fix — `tests/unit/test_routes_memory.py`
...
- **The platform's own "unbounded offset" class** (this repo's own
  ...
  already fixed it). `GET /reflections`'s `offset` had no upper bound, so a
